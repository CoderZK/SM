...
+ (YWUnlockView *)shareInstace {
    ...
    dispatch_once(&onceToken, ^{
        danli = [YWUnlock_BD loadNibNamed:@"YWUnlockView" owner:nil options:nil].lastObject;
        danli.frame = CGRectMake(0, 0, YWUnlock_SW, YWUnlock_SH);
    });
...
- (void)validateGesturesPassword:(NSString *)gesturesPassword {
    ...
    if ([gesturesPassword isEqualToString:[YWUnlockView getGesturesPassword]]) {
        ...
        if (errorCount - 1 == 0) {//你已经输错五次了！ 退出登陆！
            UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"提示" message:@"退出登录,手势密码将失效" delegate:self cancelButtonTitle:nil otherButtonTitles:@"确定", nil];
            [alertView show];
            errorCount = 5;
            return;
        }
...
+ (void)showUnlockViewWithType:(YWUnlockViewType)type callBack:(CallBackBlock)callBack{
    ...
    YWUnlockView *unlockView = [YWUnlockView shareInstace];
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    !self.forgetPassBlock?:self.forgetPassBlock(1);
    [self hide];
}
...
- (void)setType:(YWUnlockViewType)type{
    ...
    if (type == YWUnlockViewCreate) {
        [self.resetGesturesPasswordButton setTitle:@"重新绘制" forState:UIControlStateNormal];
    }else {
        [self.resetGesturesPasswordButton setTitle:@"忘记密码" forState:UIControlStateNormal];
    }
